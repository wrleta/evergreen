...
export default function Block(props: any) {
  ...
  const routes = {
    ...
    stopRequestsDetail: "/stop-requests", // Portal_Stop_Requests details
  ...
  const normalizeStatus = (raw: any) => {
    ...
    if (
      ...
      v.includes("needs info") ||
      v.includes("need info") ||
      v.includes("follow") ||
      v.includes("need")
    )
      return "Insufficient";
            ...
            {filtered.length ? (
              <div>
                {filtered.slice(0, 40).map((r: any, idx: number) => {
                  ...
                  const key =
                    toText(pick(r, ["id", "ID", "Request_ID", "Stop_Request_ID", "__ROW_NUMBER__"])) || String(idx);
