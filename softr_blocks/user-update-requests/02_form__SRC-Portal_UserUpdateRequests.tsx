...
export default function Block(props: any) {
  ...
  const routes = {
    userUpdateRequestsNew: "/user-update-requests",
    userUpdateRequestsDetail: "/user-update-requests",
  };
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((r: any) => {
      ...
      const changes = toText(pick(r, ["Requested_Field", "Current_Value", "Proposed_Value", "Reason_Text", "Requested_Changes", "Changes", "Message", "Notes", "Request_Text", "Update_Request", "Details"]));

      const phone = toText(pick(r, ["Requested_Phone", "Phone", "Phone_E164"]));
      const language = toText(pick(r, ["Requested_Language", "Language", "Lang"]));
      const crew = toText(pick(r, ["Requested_Crew_ID", "Crew_ID", "Crew", "Crew_Name"]));
      const role = toText(pick(r, ["Requested_Role", "Role"]));

      const hay = `${name} ${email} ${workerId} ${when} ${changes} ${phone} ${language} ${crew} ${role} ${st}`.toLowerCase();
  ...
  return (
    <div
      id="user-update-requests-filters"
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 rounded-xl px-3 h-11 w-full" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.10)" }}>
          ...
          <input
            ...
            placeholder={isOffice ? "Search… (name / email / changes / crew)" : "Search… (your requests)"}
          ...
          {filtered.length ? (
            <div>
              {filtered.slice(0, 40).map((r: any, idx: number) => {
                ...
                const name =
                  toText(pick(r, ["Worker_Name", "Employee_Name", "Name", "User_Name"])) ||
                ...
                const changes =
                  toText(pick(r, ["Requested_Field", "Current_Value", "Proposed_Value", "Reason_Text", "Requested_Changes", "Changes", "Message", "Notes", "Request_Text", "Update_Request", "Details"])) || "";
                ...
                const key =
                  toText(pick(r, ["id", "ID", "Request_ID", "User_Update_Request_ID", "__ROW_NUMBER__"])) || String(idx);
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {workerId ? (
                            <span className="px-2 py-1 rounded-full text-[10px] font-semibold" style={{ background: "rgba(0,233,239,0.10)", border: "1px solid rgba(0,233,239,0.18)", color: "rgba(0,233,239,0.90)" }}>
                              {workerId}
                            </span>
                          ) : null}
