...
export default function Block(props: any) {
  ...
  // Worker safety filter (cosmetic only; Softr should filter at source ideally)
  ...
  const scrollToFilters = () => {
    try {
      const el = document.getElementById("stop-requests-filters");
      if (el) return el.scrollIntoView({ behavior: "smooth", block: "start" });
      window.location.hash = "stop-requests-filters";
    } catch (e) {}
  };

  // ---------- VISUAL (no full-screen bg; Softr/app provides global bg) ----------
  const banner = {
        ...
        <div className="mx-auto w-full max-w-5xl px-4 sm:px-6" style={{ paddingTop: 12, paddingBottom: 12 }}>
          <Card className="rounded-2xl" style={cardStyle}>
            <CardContent className="py-12 text-center">
              <ClipboardList className="w-20 h-20 mx-auto" style={{ color: "rgba(0,233,239,0.55)" }} />
              <div className="mt-4 text-sm" style={{ color: "rgba(229,231,235,0.75)" }}>
                Please log in to view stop requests.
              </div>
      ...
      <div className="mx-auto w-full max-w-5xl px-4 sm:px-6" style={{ paddingTop: 12, paddingBottom: 12 }}>
        ...
        <div className="space-y-2">
          <div className="flex items-end justify-between gap-3">
            <div className="min-w-0">
              <div className="text-2xl font-extrabold leading-tight truncate text-white">Stop Requests</div>
              <div className="text-sm font-semibold" style={{ color: "rgba(229,231,235,0.72)" }}>
                Missing or wrong stops (triage lane)
              </div>
            </div>

            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.10)" }} title="Stop Requests">
          ...
          {/* Nav row (4-col): Filter (scroll) / Reports / Today / FAQ */}
          <div className="grid grid-cols-4 items-center" style={{ paddingTop: 10, gap: 8 }}>
            <button style={linkBtn} onClick={scrollToFilters}>
              ...
              <span style={{ fontWeight: 800 }}>
                Filter <span style={thinGray}>(below)</span>
              </span>
            </button>
        ...
        <Card className="rounded-2xl mt-4" style={cardStyle}>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="text-xs" style={{ color: "rgba(229,231,235,0.55)", fontWeight: 500 }}>
